...
Originating backtests remain descriptive development evidence. Dedicated Strategy Validation records the development cutoff and period reuse, freezes a recent holdout, supports fixed-parameter rolling/expanding walk-forward windows, excludes warm-up from scored metrics, stresses costs/delays/nearby parameters without selecting on holdout, and records repeated holdout access. See `docs/validation.md`. No result proves future profitability.
